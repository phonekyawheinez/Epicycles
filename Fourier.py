...
class Fourier(object):
    def __init__(self, n_approx=1000, coord_1=None, coord_2=None, low_pass_threshold=None):
        ...
        if coord_2 is not None:
            ...
            if self.complex_coord_2.size > self.complex_coord_1.size:
                self.complex_coord_1 = np.hstack((self.complex_coord_1,
                                                  np.full((self.complex_coord_2.size - self.complex_coord_1.size),
                                                          self.complex_coord_1[-1], dtype=np.complex_)))
            elif self.complex_coord_1.size > self.complex_coord_2.size:
                self.complex_coord_2 = np.hstack((self.complex_coord_2,
                                                  np.full((self.complex_coord_1.size - self.complex_coord_2.size),
                                                          self.complex_coord_2[-1], dtype=np.complex_)))
        else:
    ...
    def get_one_circle_one_image(self):
        ...
        circles_loc = np.zeros((2 * (self.n_approx - 1), period), dtype=np.complex128)
        circles_rad = np.zeros((2 * (self.n_approx - 1)), dtype=np.float64)

        fourier_coeff_1 = self.fft(self.complex_coord_1)

        for idx, multiple in enumerate(chain(range(-self.n_approx + 1, 0), range(1, self.n_approx))):
            cn = fourier_coeff_1[multiple]
    ...
    def get_two_circles_one_image(self):
        ...
        for idx, multiple in enumerate(range(1, self.n_approx)):
            cn_1 = fourier_coeff_1[multiple].real
            an_1, bn_1 = cn_1, cn_1.imag
            ...
            circles_loc_1[idx + self.n_approx - 1, :] = circles_loc_1[idx + self.n_approx - 1,
                                                        :].imag + 1j * circles_loc_1[idx + self.n_approx - 1, :].real

            cn_2 = fourier_coeff_1[multiple].imag
            ...
            circles_loc_2[idx + self.n_approx - 1, :] = circles_loc_2[idx + self.n_approx - 1,
                                                        :].imag + 1j * circles_loc_2[idx + self.n_approx - 1, :].real
    ...
    def get_two_circles_two_images(self):
        ...
        fourier_coeff_1 = self.fft(self.complex_coord_1)
        fourier_coeff_2 = self.fft(self.complex_coord_2)

        for idx, multiple in enumerate(chain(range(-self.n_approx + 1, 0), range(1, self.n_approx))):
            cn_1 = fourier_coeff_1[multiple]
            cn_2 = fourier_coeff_2[multiple]
    ...
    def fft(self, coordinates):
        fourier_coefficients = np.fft.fft(coordinates)

        if self.low_pass_threshold is not None:
            # low-pass filter
            fourier_coefficients[np.abs(np.fft.fftfreq(len(coordinates))) > self.low_pass_threshold] = 0
